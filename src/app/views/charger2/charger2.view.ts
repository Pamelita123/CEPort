...
export class Charger2View implements OnInit, OnDestroy, AfterViewInit {
  ...
  rows: TableRow[] = [];
  private pollId: any;
  private readonly FEED_KEY = 'ultrasonic-distance2';
  ...
  async ngOnInit(): Promise<void> {
    try {
      ...
      console.error('No se pudo cargar el nombre del sensor (ultrasonic-distance2):', e);
    }
  }
  ...
  async onEdit(row: TableRow) {
    ...
    ref.afterClosed().subscribe(async (result) => {
      ...
      try {
        ...
        console.error('Error updating ultrasonic-distance2 data:', e);
