...
@Component({
  ...
  styleUrls: ['./humidity.view.scss'],
  imports: [CommonModule, NavBar, LoaderComponent, TableComponent],
  standalone: true
})
export class HumidityView implements OnInit, OnDestroy, AfterViewInit {
  columns: TableColumn[] = [
    ...
    { key: 'value', header: 'Valor (%)', type: 'number', width: 140 },
    { key: 'id', header: 'ID', type: 'text', width: 320 }
  ];

  rows: TableRow[] = [];
  private pollId: any;
  private readonly FEED_KEY = 'humidity';
  ...
  private async loadRows() {
    try {
      ...
      console.error('Error loading humidity data:', e);
      this.rows = [];
      this.cdr.detectChanges();
    }
  }
